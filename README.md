...
| **Ancient Fortune** | capstone | Lifts your ceiling on vanilla Fortune from III to IV — your table can roll it and your anvil can forge it; a Fortune IV tool is inert in the hands of anyone without the node |
...
| **Reaper’s Wisdom** | capstone | A mob killed with a Looting weapon gives +25% XP per Looting level, all four of them — +100% at Looting IV. Stacks on top of Scholar |
| **Prospector’s Wisdom** | capstone | The mining half of the same idea: a block broken with a Fortune tool gives +25% XP per Fortune level, all four of them — +100% at Fortune IV. Stacks on top of Scholar |
...
**Greater Mending.** The Mending data file raises `max_level` to 2 for everybody, because a data pack cannot be per-player — the same trick as Ancient Fortune. What makes it a reward is that the anvil will not *forge* Mending II for anyone without the node, and a Mending II item that reaches such a player anyway is inert in their hands (see *shared items* below). The anvil is the only place Mending II can be made at all, since Mending is treasure and never rolls at a table.
...
| **Third Eye** | 5 | The Seeker's Eye, looking through walls: while the magnifier holds a search, every container in the 3×3×3 chunks around you (your chunk and its ring, one layer up and one down) holding a match glows through everything — chests, placed shulkers, furnaces, hoppers — each traced as the silhouette your eye would draw, not a full wireframe. Walk and new finds light up as they enter reach; the ones you leave behind go dark. The height cut is deliberate: it confirms your own storage, it does not read the dungeon under the floor |
...
**The three capstones are a real pick-one.** Aegis lifts your ceiling on vanilla **Protection from IV to V** — raised in the data pack for everybody, the same trick Ancient Fortune and Greater Mending use; for anyone without the node the anvil will not forge a V out of two IVs, and a Protection V piece that reaches them anyway (rolled at a table, bought, gifted) keeps its rank and protects like bare skin until they buy Aegis. Immortal Line turns a killing blow into one heart with a few seconds of Regeneration and Resistance, once every ten minutes, consuming no totem. Living Armor puts the experience you pick up into the whole set at once, from any source and with no Mending on the pieces. Buying one puts the other two out of reach, which is what made a node's exclusivity a *list* rather than a single id.

**Shared items.** The passives are on the player, not on the gear, and the five enchantments the tree hands out are ordinary enchantments that anyone can carry once they exist — but since 0.8.4 armour is under the inert-item rule like everything else: a worn piece carrying a rank its wearer has not earned (Protection V without Aegis, or one of the tree's own ranks above theirs) adds no armour points, no toughness and no enchantment protection, and takes no wear, until the rank is bought. The other pieces keep protecting normally.
...
| **Hunter's Bounty** | 7 | Capstone: vanilla **Power** reaches VI at the table and the anvil — the Ancient Fortune trick; a Power VI bow is inert for anyone without the node |
...
> Hold an item carrying a **rank** you have not unlocked, and it behaves as if your hands were empty.
...
- ✅ **Partial ownership is not ownership.** Own Dig Range II, hold a Dig Range III pickaxe, and it is a stick — not a rank II pickaxe. The rank on the label is the rank you have to have bought, and it does not matter which of the three routes put it there: the enchanting table, a librarian's book or another player's generosity. Buy rank III and the tool wakes up whole.
...
- ✅ **Creative mode bypasses it**, so testing every other feature does not mean grinding the tree first. It is the one hole on purpose, and it is why an unearned rank looks like it works when you are flying: use `/mastery debug unlockall` and survival to check the gate itself.
...
**Not covered.** Vanilla enchantments within vanilla's own ceilings have no unlock concept, so a plain Efficiency V pickaxe still transfers freely, and vanilla's own attack-speed attribute is untouched; only the five ceilings the mod raises (Fortune IV, Looting IV, Protection V, Power VI, Mending II) are gated. Elytra are out of scope. Armour joined in 0.8.4 — an inert piece protects like bare skin; bows grew their own branch with the Bow tree — an unearned bow still fires, at bare-bow damage with no enchantment effects. The check is written generally enough that "inert" can mean something different per item class.
