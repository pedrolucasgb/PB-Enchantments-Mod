...
public final class SkillService {
	...
	public static Result unlockNextTier(ServerPlayer player, SkillTree tree) {
		...
		if (next >= tree.tiers().size()) {
			return fail("msg.pbenchants.tier.fail.complete");
		}
		SkillTier tier = tree.tiers().get(next);
		if (!gateComplete(progress, tier)) {
			return fail("msg.pbenchants.tier.fail.gate", tree.id());
		}
		if (player.experienceLevel < tier.accessCost()) {
			return fail("msg.pbenchants.fail.levels", tier.accessCost(), player.experienceLevel);
		}
		player.giveExperienceLevels(-tier.accessCost());
		progress.unlockedTiers = next + 1;
		ModAdvancements.grantTier(player, tree.id(), next);
		return ok("msg.pbenchants.tier.ok", next + 1, tier.accessCost());
	}
	...
	public static Result unlockNode(ServerPlayer player, SkillTree tree, SkillNode node) {
		TreeProgress progress = progress(player, tree);
		if (!node.implemented()) {
			return fail("msg.pbenchants.unlock.fail.future", node.displayName());
		}
		if (progress.owns(node.id())) {
			return fail("msg.pbenchants.unlock.fail.owned", node.displayName());
		}
		if (node.tier() >= progress.unlockedTiers) {
			return fail("msg.pbenchants.unlock.fail.tier", node.tier() + 1);
		}
		if (node.requires() != null && !progress.owns(node.requires())) {
			return fail("msg.pbenchants.unlock.fail.requires", SkillNode.displayName(node.requires()));
		}
		String blocker = node.blockedBy(progress::owns);
		if (blocker != null) {
			return fail("msg.pbenchants.unlock.fail.exclusive", SkillNode.displayName(blocker));
		}
		if (node.requiresAll()) {
			int missing = tree.missingForCompletion(progress.purchased, node.id());
			if (missing > 0) {
				return fail("msg.pbenchants.unlock.fail.requires_all", missing);
			}
		}
		MaterialCost missing = MaterialCost.missing(player, node.materials());
		if (missing != null) {
			return fail("msg.pbenchants.unlock.fail.materials", missing.label(), missing.held(player));
		}
		if (player.experienceLevel < node.unlockCost()) {
			return fail("msg.pbenchants.fail.levels", node.unlockCost(), player.experienceLevel);
		}
	...
	private static Component unlockMessage(SkillNode node) {
		if (!node.enchantable()) {
			return Component.translatable(node.type() == SkillType.PASSIVE
				? "msg.pbenchants.unlock.ok.passive"
				: "msg.pbenchants.unlock.ok.other", node.displayName());
		}
		...
		return Component.translatable(inTable
				? "msg.pbenchants.unlock.ok.enchantment"
				: "msg.pbenchants.unlock.ok.capstone",
	...
	public static Result enchantHeld(ServerPlayer player, SkillTree tree, SkillNode node) {
		...
		if (!progress(player, tree).owns(node.id())) {
			return fail("msg.pbenchants.enchant.fail.locked", node.displayName());
		}
		ModEnchantments.Grant grant = ModEnchantments.NODE_GRANTS.get(node.id());
		...
		if (player.experienceLevel < node.enchantCost()) {
			return fail("msg.pbenchants.fail.levels", node.enchantCost(), player.experienceLevel);
		}
	 ...
	 * unlocked, not applied — use {@code /pbenchants debug kit} for tools that
	...
	public static Result unlockTierNodes(ServerPlayer player, SkillTree tree, int tierNumber) {
		...
		// Complete the gates up to here too, so /pbenchants status and the tier
		...
		ModAdvancements.syncAll(player);
		return ok("msg.pbenchants.tier.unlocked", granted, tierNumber, tree.id(), skipped);
	}
	...
	public static Result lockNode(ServerPlayer player, SkillTree tree, SkillNode node) {
		...
		return ok("msg.pbenchants.lock.ok", node.displayName());
	}
	...
	public static Result setTier(ServerPlayer player, SkillTree tree, int tiers) {
		...
		ModAdvancements.syncAll(player);
		return ok("msg.pbenchants.tier.set", tree.id(), tiers);
	}

	/**
	 * Debug: strips every PB Enchantments enchantment off the held item, so the
	 * same tool can be fed back through the Enchant button.
	 */
	public static Result stripHeld(ServerPlayer player) {
		ItemStack stack = player.getMainHandItem();
		if (stack.isEmpty()) {
			return fail("msg.pbenchants.strip.fail.empty");
		}
		int removed = 0;
		...
		return ok("msg.pbenchants.strip.ok", removed, stack.getHoverName());
	}
