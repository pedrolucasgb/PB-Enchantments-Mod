...
public final class ArtisanHandler {
	...
	private static void cycleSortMode(ServerPlayer player) {
		...
		player.sendSystemMessage(Component.translatable("screen.pbenchants.sort.now", mode.label())
	...
	private static void quickStack(ServerPlayer player) {
		...
		report(player, outcome, "perk.pbenchants.hand_of_order.done", "perk.pbenchants.hand_of_order.nothing");
	}
	...
	private static void restock(ServerPlayer player) {
		...
		report(player, outcome, "perk.pbenchants.restock.done", "perk.pbenchants.restock.nothing");
	}
