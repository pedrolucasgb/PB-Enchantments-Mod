package dev.pbenchants.command;
 ...
 *
 *   /pbenchants status <tree>
 *   /pbenchants unlock <tree>            unlock the next tier
 *   /pbenchants unlock <tree> <node>     unlock a node (XP levels + materials)
 *   /pbenchants enchant <tree> <node>    stamp it on the held item (XP levels, repeatable)
 *   /pbenchants debug add <tree> <counter> <amount>   (op only)
 *
 ...
 */
public final class PBEnchantsCommand {
	...
	private PBEnchantsCommand() {
	}

	public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
		dispatcher.register(Commands.literal("pbenchants")
			...
			.then(Commands.literal("debug")
				...
				.then(Commands.literal("unlockall")
					.executes(context -> {
						...
						dev.pbenchants.network.ModNetworking.sendState(player);
						context.getSource().sendSystemMessage(Component.literal("Every node unlocked. Enchantments are unlocked only - stamp them on a tool with /pbenchants enchant, the skill screen, or /pbenchants debug kit.").withStyle(ChatFormatting.YELLOW));
						return 1;
					}))
		...
		// The mod used to be called Tool Mastery; keep /pbenchants answering so
		// muscle memory gets a pointer to the new command instead of an error.
	...
	private static int legacyNotice(CommandSourceStack source) {
		source.sendSystemMessage(Component.translatable("msg.pbenchants.legacy_command")
	...
	private static int status(CommandSourceStack source, String treeId) {
		...
		if (next < tree.tiers().size()) {
			...
			source.sendSystemMessage(Component.literal("Next gate (tier " + (next + 1) + ", access "
					+ dev.pbenchants.skill.XpMath.pointsForLevel(tier.accessCost()) + " XP):")
	...
	/** /pbenchants debug unlocktier <tree|all> <n> - hand over one whole tier, gates and nodes included. */
	...
	/**
	 * /pbenchants debug master true|false - free-unlock mode for testing. Every
	...
	private static int master(CommandSourceStack source, boolean enabled) {
		...
		dev.pbenchants.progress.ModAttachments.of(player).debugMaster = enabled;
		dev.pbenchants.network.ModNetworking.sendState(player);
	...
	/** /pbenchants debug reset [tree] - wipe progress back to a brand-new player. */
	private static int reset(CommandSourceStack source, @Nullable String treeId) {
		...
		source.sendSystemMessage(Component.literal(tree == null
				? "Everything reset: no tiers, no nodes, no gate counters, tier advancements revoked. Enchantments already on tools stay - clear those with /pbenchants debug strip."
				: "Tree '" + tree.id() + "' reset: no tiers, no nodes, no gate counters.")
			.withStyle(ChatFormatting.YELLOW));
		return 1;
	}

	/** /pbenchants debug lock <tree> <node> - re-lock one node, leaving the rest alone. */
	...
	/** /pbenchants debug tier <tree> <n> - open exactly n tiers, re-locking anything above. */
	...
	/**
	 * The blocks {@code /pbenchants debug speed} measures: one per speed passive it
	...
	/**
	 * /pbenchants debug speed - what the speed passives are actually doing to the
	...
	private static int speed(CommandSourceStack source) {
		...
		for (net.minecraft.world.level.block.Block block : SPEED_SAMPLES) {
			...
			float factor = dev.pbenchants.perk.MiningSpeed.multiplier(player, state);
	...
	/**
	 * /pbenchants debug kit [tree] [enchant]
	...
	private static int kit(CommandSourceStack source, @Nullable String treeId, @Nullable String enchantName) {
		...
		dev.pbenchants.enchant.ModEnchantments.Grant reference = null;
		for (int level = 1; level <= maxLevel; level++) {
			...
			dev.pbenchants.enchant.ModEnchantments.apply(player, stack, reference.enchantment(), level);
	...
	private static void giveTierTools(ServerPlayer player, SkillTree tree, net.minecraft.world.item.Item item, String label) {
		for (int tierIndex = 0; tierIndex < tree.tiers().size(); tierIndex++) {
			...
			best.forEach((enchantmentKey, level) ->
				dev.pbenchants.enchant.ModEnchantments.apply(player, stack, enchantmentKey, level));
			...
			// lifts.
			dev.pbenchants.enchant.ModEnchantments.apply(player, stack,
