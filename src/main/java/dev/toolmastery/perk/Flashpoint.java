...
public final class Flashpoint {
	...
	public static boolean allowDamage(LivingEntity entity, DamageSource source, float amount) {
		...
		if (state.armed) {
			...
			say(player, "perk.toolmastery.flashpoint.open", ChatFormatting.GOLD);
		}
	...
	public static void tick(ServerPlayer player) {
		...
		if (state.ticksLeft > 0) {
			...
			if (state.ticksLeft > 0 && state.ticksLeft % 20 == 0) {
				player.sendSystemMessage(Component.translatable("perk.toolmastery.flashpoint.countdown",
					Component.literal(String.valueOf(state.ticksLeft / 20)).withStyle(ChatFormatting.GOLD)), true);
			} else if (state.ticksLeft == 0) {
				say(player, "perk.toolmastery.flashpoint.spent", ChatFormatting.RED);
			}
		}
		...
		if (!state.armed && !player.isOnFire() && !player.isInLava()) {
			state.armed = true;
			state.ticksLeft = 0;
			say(player, "perk.toolmastery.flashpoint.ready", ChatFormatting.AQUA);
		}
	}
