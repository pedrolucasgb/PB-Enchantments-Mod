 ...
 *
 * Passives that change block-breaking speed have to answer the same on both
...
public final class PerkAccess {
	/** Reads the client's synced snapshot. Answers false until the client installs one. */
	public interface Lookup {
		boolean owns(String treeId, String nodeId);
	}

	private static Lookup clientLookup = (treeId, nodeId) -> false;
	...
	public static boolean owns(Player player, SkillTree tree, String nodeId) {
		...
		return clientLookup.owns(tree.id(), nodeId);
	}
