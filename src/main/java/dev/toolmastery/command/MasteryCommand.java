...
public final class MasteryCommand {
	...
	public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
		dispatcher.register(Commands.literal("mastery")
			...
			.then(Commands.literal("unlock")
				.then(Commands.argument("tree", StringArgumentType.word())
					.suggests(TREE_IDS)
					.executes(context -> unlock(context.getSource(), StringArgumentType.getString(context, "tree")))))
			...
			.then(Commands.literal("debug")
				...
				.then(Commands.literal("buyall")
					.executes(context -> {
						...
						SkillService.maxAll(player);
						SkillService.buyAll(player);
						dev.toolmastery.network.ModNetworking.sendState(player);
						context.getSource().sendSystemMessage(Component.literal("All nodes granted. Enchantment nodes applied to your held tool when compatible; otherwise use /enchant.").withStyle(ChatFormatting.YELLOW));
						return 1;
					}))
	...
	private static int unlock(CommandSourceStack source, String treeId) {
		SkillTree tree = treeOrFail(source, treeId);
		ServerPlayer player = source.getPlayer();
	...
	private static int buy(CommandSourceStack source, String treeId, String nodeId) {
		SkillTree tree = treeOrFail(source, treeId);
		ServerPlayer player = source.getPlayer();
		if (tree == null || player == null) {
			return 0;
		...
		return report(source, SkillService.buyNode(player, tree, node));
	}
	 ...
	 */
	private static int kit(CommandSourceStack source, @org.jetbrains.annotations.Nullable String treeId,
	                       @org.jetbrains.annotations.Nullable String enchantName) {
		ServerPlayer player = source.getPlayer();
		if (player == null) {
			return 0;
		}
	...
	private static void giveTierTools(ServerPlayer player, SkillTree tree, net.minecraft.world.item.Item item, String label) {
		for (int tierIndex = 0; tierIndex < tree.tiers().size(); tierIndex++) {
			...
			// (tier 5 = Fortune IV, the Ancient Fortune capstone).
	...
	private static int report(CommandSourceStack source, SkillService.Result result) {
		switch (result) {
			case SkillService.Result.Ok ok -> source.sendSystemMessage(Component.literal(ok.message()).withStyle(ChatFormatting.GREEN));
			case SkillService.Result.Fail fail -> source.sendFailure(Component.literal(fail.message()));
		}
