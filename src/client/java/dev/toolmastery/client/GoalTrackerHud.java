...
public final class GoalTrackerHud {
	private static final Identifier ID = Identifier.fromNamespaceAndPath(ToolMastery.MOD_ID, "goal_tracker");
	...
	private static List<Line> tierLines(SkillTree tree, int tierIndex, SkillStatePayload.TreeState state) {
		...
		lines.add(new Line(tree.shortName().getString() + " — "
			+ Component.translatable("hud.toolmastery.track.tier", tierIndex + 1).getString(), SkillTreeStyle.MUTED));
		if (tierIndex < state.unlockedTiers()) {
			lines.add(new Line(Component.translatable("hud.toolmastery.track.unlocked").getString(),
				SkillTreeStyle.GREEN));
			return lines;
		}
		SkillTier tier = tree.tiers().get(tierIndex);
		if (addGateLines(lines, tier, state)) {
			lines.add(new Line(Component.translatable("hud.toolmastery.track.gate_done",
